...
class Actor(torch.nn.Module, PrintParamCountMixin, metaclass=PostInitCaller):
    ...
    def _normalized_obs(self, obs: deque, flatten: bool = True):
        ...
        if self.observation_type == "image":
            ...
            # nobs = torch.cat([nrobot_state, part_poses_top, part_poses_leg4], dim=-1)
    ...
    def _training_obs(self, batch, flatten: bool = True):
        ...
        if self.observation_type == "image":
            ...
            # robot_state = batch["obs"][:, : self.obs_horizon, :16]
            # part_poses_top = batch["obs"][:, : self.obs_horizon, 16:23]
            # part_poses_leg4  = batch["obs"][:, : self.obs_horizon, -14:-7]
            # nobs = torch.cat([robot_state, part_poses_top, part_poses_leg4], dim=-1)
