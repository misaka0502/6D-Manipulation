...
def main(cfg: DictConfig):
    ...
    if run_exists:
        ...
        run_state_dict = torch.load(wts, map_location="cuda:7")
    ...
    while global_step < cfg.total_timesteps:
        ...
        for epoch in trange(cfg.update_epochs, desc="Policy update"):
            ...
            for start in range(0, cfg.batch_size, cfg.minibatch_size):
                ...
                # print("--- Checking Optimizer State Devices ---")
                # found_state_on_cpu = False
                # if optimizer_actor.state: # 检查 state 是否为空
                #     for group in optimizer_actor.param_groups:
                #         for p in group['params']:
                #             if p in optimizer_actor.state:
                #                 param_state = optimizer_actor.state[p]
                #                 for key, value in param_state.items():
                #                     if isinstance(value, torch.Tensor):
                #                         if value.device != device:
                #                             print(f"ERROR: Optimizer state '{key}' for a parameter is on device: {value.device} (Expected: {device})")
                #                             found_state_on_cpu = True
                #                             # break # 可以提前退出内层循环
                #         # if found_state_on_cpu: break # 可以提前退出外层循环

                #     if not found_state_on_cpu:
                #         print("All existing optimizer state tensors are on the target device.")
                #     else:
                #         print("ERROR: Found optimizer state tensor(s) not on the target device!")
                # else:
                #     print("Optimizer state is empty (normal on first step or if state is not kept).")
                # print("------------------------------------")
